...
- `400` — missing `sessionId` for `GET /api/chat/session-insights`
...
- `400` — missing `sessionId` for `GET /api/chat/session-tools`
...
- `409` — `provider_env_not_syncable` when the provider is env-configured but cannot be synced into Pi runtime auth storage
- `409` — `signin_attempt_not_active` when manual input or cancel is sent to a finished or stale signin attempt
- `409` — `provider_not_authenticated` when chat execution uses a signed-off auth-backed provider
...
- `404` — `session_insights_unavailable` when the local Pi session file is unavailable
- `404` — `session_not_found` or `session_metadata_missing` for `GET /api/chat/session-model`
...
- `500` — `storage_usage_unavailable` when `GET /api/storage/usage` fails unexpectedly
