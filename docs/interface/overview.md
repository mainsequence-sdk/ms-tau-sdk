...
Clients must identify the backend `Agent.agent_type` (`agentType`) on chat requests.
Chat and stateless LLM requests use `user_uid`; model/provider control-plane endpoints use
`created_by_user_uid`.
