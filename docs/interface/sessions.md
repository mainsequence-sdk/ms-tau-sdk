...
`runtime_session_id` is the backend `AgentSession.id` string.
...
- Real `POST /api/chat` and `POST /api/a2a/chat` requests must include `runtime_session_id` (or an
  accepted alias such as `sessionId` or `runtimeSessionId`).
- Astro attaches to that existing backend session and reuses its `AgentSession.id` as the runtime
  session key.
...
- If `runtime_session_id` is provided for `astro-orchestrator` and local wrapper files are
...
`runtime_session_id` from the backend control-plane step that created the session before Astro was
called.
