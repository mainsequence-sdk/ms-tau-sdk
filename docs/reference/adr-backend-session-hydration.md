...
- the backend session record resolves to `agent_type = astro-orchestrator`
...
- `agentUniqueId` from the incoming request user id plus `agentType`
...
- `agentUniqueId` from the requested user id plus `agentType`
...
- the backend session exists but does not resolve to `agent_type = astro-orchestrator`
...
3. Call hydration from the resume path when:
   ...
   - backend registration is enabled
4. Only proceed when the backend session resolves to `agent_type = astro-orchestrator`.
5. Do not call `registerMainsequenceAgent(...)` or `startBackendAgentSession(...)` on this path.
6. Reuse the existing request-scoped `latestUserMessage`, `agentType`, `userId`, and optional
