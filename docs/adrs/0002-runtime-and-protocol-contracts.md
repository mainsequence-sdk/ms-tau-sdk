...
4. Sessionless agent responses construct a direct Tau `AgentHarness`. They do not create durable
   sessions, tasks, snapshots, or project tools.
5. The SDK preserves its tested chat, response, session, health, readiness, A2A REST/JSON-RPC, SSE,
...
Contract and unit suites cover authentication, provider evidence, MCP, durable/sessionless paths,
